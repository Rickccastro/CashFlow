...
public class UpdateExpenseTest : CashFlowClassFixture
{
    ...
    public async Task Error_Title_Empty(string cultureinfo)
    {
        ...
        var result = await DoPut(requestUri: $"{METHOD}/{_expenseId}", request: request, token: _token,culture: cultureinfo);
    ...
    [Theory]
    [ClassData(typeof(CultureInlineDataTest))]
    public async Task Error_Expense_Not_Fond(string cultureinfo)
    {
        var request = RequestExpenseJsonBuilder.Build();
        ...
        var expectedMessage = ResourceErrorMenssages.ResourceManager.GetString("EXPENSE_NOT_FOUND", new CultureInfo(cultureinfo));
